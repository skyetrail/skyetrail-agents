...
| [`auditing-skills`](plugins/steering/skills/auditing-skills/SKILL.md) | [`steering`](plugins/steering) | 1.0.0 | Audits any document written to steer an agent against the house rules, and reports what to fix, marking each finding blocking, important, or advisory. Use this whenever someone asks to review, check, audit, lint, or sanity-check a skill, a SKILL.md, a slash command, a prompt written for a subagent, a hand-off brief, a runbook, or an AGENTS.md or CLAUDE.md instruction file, when they want to know why a skill is not triggering or not being followed, why a subagent came back with nothing useful, or when a skill is about to ship. |
