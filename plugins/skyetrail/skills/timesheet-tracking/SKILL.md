...
Keep a project timesheet up to date. Each entry is one work session: the date, the start and end time, the duration, and a short description. This skill finds or creates the timesheet, adds an entry from what the user gives, and can edit an entry.
...
- Newest entry first: add new rows at the top, right below the header.
...
- [ ] Step 5: Add the row and show the summary
```
...
**Step 5: Add the row and summarize**
Add the row at the top of the table, right below the header. Then show the summary described below.

## Edit an entry
1. Ask which entry, or find it from the date and description the user gives.
2. Show the current row.
