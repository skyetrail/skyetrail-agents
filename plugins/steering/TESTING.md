...
Mechanical checks run through two commands, listed below.
...
3. Move any earlier output for the same task where this run cannot read it.
...
| --- | --- |
| Did it deliver? | Search each artifact for `unverified`, `draft`, `do not use`, `provisional`. A file whose own text says it is not the deliverable does not count as delivered. |
...
| What went missing? | List what the unaided run names and the skilled run does not. Read unwrapped text. A wrapped line can hide missing content. |
...
Contamination hit two rounds. Both are recorded.

**A shared scratchpad.** One directory saw six runs use it. One fixture had three runs share a
record path and a draft path, and two of them read the first run's output before they wrote. One
said so. Agreement between those runs is co-authorship, not convergence.
...
3. Trace each overlapping span to a source. Quoting from one rule file across two runs counts as
   shared source, not contamination.
...
The gate round re-ran seven claims, of which six reproduced exactly: three audit counts, and three
checksums with their hole counts and line numbers. The seventh failed, and that is the false tick
above.
...
- Review this SKILL.md before I release it.
...
- We have a reviewer agent, I want to compose it per call instead.
...
`writing-agents` and `writing-skills` both changed on 2026-08-12. `tests/baselines/` holds four
records describing earlier versions of those files. Do not cite one as current.
...
loaded, and compare. Where behaviour is the same either way, the skill adds nothing.
...
Biased scoring is blocked by four rules, listed below.

1. The answer key never appears in any prompt.
2. Whoever scores a run is not the agent that ran it.
3. A run file contains an opaque identifier, and the mapping to its arm is kept where the scorer
   cannot read it. Check the blind held before you score.
...
This name covers two different things.
