...
use `<skill-name>/` under the current working directory and name it in your report. A person
installs the skill by moving the `<skill-name>/` directory, so nothing installs it on its own. Where
the skill already exists, edit it where it is.
...
a value the skill leaves to the reader's setup. A step you could not run is a line in `record.md`,
and never a reason to hold the file back.
...
run that proves the line is done. The skill's own text proves nothing, and neither does a rule file.
For a step about applying a rule file, the tick names the sections of the delivered SKILL.md where
those rules land. A line you cannot tick stays unticked and carries one line saying why.
...
3. **Anchor the objective.** Copy the person's request into `record.md` under `## Objective`, word
   for word.
4. **List what you know about the subject** before you apply any rule. The section "Structure versus
   subject matter" in `../../shared/authoring.md` states the step. The rule files carry the
...
6. **Number the misses.** A miss is anything the run produced that a person must correct before
   ...
   word. A miss you cannot quote is not a miss. Where step 5 was blocked, write `misses unnumbered`
   and go on.
...
10. **Run `npm run audit -- <delivered path>`** from the root of this plugin's repository, and paste
    ...
    The path on its `SKILL.md` line matches the `delivered` line character for character, or it
    measured another file. Audit the delivered file where it is, never a copy.
...
dispatch still delivers, with a `blocked` line for each step that did not run, and says in its
report that no baseline measured the skill.
...
final output. That block is what the caller compares against.
