...
a small change, where step 2 governs the lines the change does not touch. A step you could
...
4. **List what you know about the subject** before you apply any rule. The section "Shape versus
   ...
   back what it dropped, except an item the baseline got right, which step 8 leaves out.
...
8. **Write the body.** In this step, the reader is the agent that will run the new skill.
   ...
   - Put a membership test beside every category the skill names, and mark every list as
     examples.
   ...
   - Move detail one case needs into `reference/`, with an instruction naming the path at the
