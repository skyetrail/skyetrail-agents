...
can name would need different text there. Name those two call sites in `record.md`, which the workflow
creates. Where only one
...
every call site, and `../../../shared/dispatch-protocol.md` keeps the set of fields a template's callers establish
fixed, so a new hole needs a new field.
