...
| A script makes every determination that does not need judgement. Nothing else makes one. | Important |
...
| Where a predefined agent is dispatched, one defined in a static file such as `agents/*.md`, the document includes no context that call does not need. | Advisory |
