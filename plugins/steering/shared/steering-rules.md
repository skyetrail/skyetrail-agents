...
summarises. A transcript. A dataset. A report an agent produced. To decide a case, ask whom the text
addresses. Apply these rules to text that tells an agent how to work, and never to material the
agent works on.
...
the same answer. A count that has nothing quoted behind it records one auditor's reading, and no
other auditor can repeat it. This default holds for any audit that reads this file, whichever skill
runs it. The skill running the audit says how to mark and count what you record.
...
You decide every condition from what the document contains. Check any statement the document makes
about which conditions it meets, because otherwise an author switches off a rule by writing one
...
because one of the rules below tests exactly that. If you decide the condition from the property
that rule tests, the condition is false wherever the rule would fail. That rule then applies only
where it passes, so it can never fail. The same is true for any rule whose condition you decide from
the property that rule tests.
...
answers, and do not infer one answer from the other.
...
instruction.** Each section in both shows the wording that passes. Where a section contains a
failing example written as an instruction, a run that needs that section copies it. One run lifted a
failing Finish example near verbatim, and its own record named the passing one as its model. Where a
short phrase must show the fault, put the label inside the quoted line. A run that copies the line
then copies the label with it.
...
injection. It filed that finding out of scope, because the line did not name markup. Each kind on
the list was correct. The one fault was that the list was unmarked.
...
| What decides the check is a value that anyone else can confirm against the material. An entry, a verdict, or a count is not such a value. | Blocking | **judges only** |
...
a claim. The work never ends on a claim. A failed gate changes the status the run reports, and the
run still delivers the artifact.
...
the delivered file, and that result came out the same every time. That lint command worked as a
gate, because the run and the caller could both run it on the same file.
...
and recorded the pass. The check was real, and its answer was true of the second file, but not of
the artifact the caller got. Another run copied its draft to a path built to satisfy a name check,
...
runs answers the first. For work that meets **judges only**, every check the agent runs leaves the
second open. So the rows conditioned on **judges only** ask for a check on what the agent covered,
and for a sentence saying what a pass leaves open.
...
prompt to migrate every call site can measure itself by one report line per call site. Every line
can report a call site that the run did not edit, and the tally still comes out whole.

**Decide these rows from the text alone.** An author who describes a run that passes the check and
stops short can still keep the check, and one run did. That description helps an author write a
check. An auditor still decides every row above from the text, without the author's agreement.

**Coverage that comes from the material is fixed before the agent acts, so no later choice changes
it.** Where the coverage comes from the agent's own findings, a run that finds nothing passes with
an empty list. One run took a passing example and moved its coverage onto the agent's findings that
way. What the agent opens and what the agent writes are two more choices of that kind, and not the
whole set.
...
conclusion beside the value is welcome, but anyone else can check only the value.

**Where a document states a gap in its check, judge the check as written.** One row asks the
document to say what a pass leaves open. That row is not a place to record that the check misses the
outcome. One run described a run that passes its own check and misses the vulnerability. It kept the
check, and wrote that the gap was disclosed rather than hidden. Judge every row above on the text it
names, because writing down a weakness does not remove it.

**A count of the writes answers the row about written state.** It answers no other row here, because
whether a write happened and whether a judgement finished are separate facts.

A check the agent runs itself need not be a script. It can also be a question the agent answers by
...
finish check as the definition of done, so the next reader of that check treats reading outside the
...
Write this kind of description only after a run shows a miss, because one written before that is a
guess, and it costs as much context as one taken from an observed failure.
...
A sentence fails the first rule where it instructs and its subject is neither the reader nor an
actor that the sentence names and that can choose.
...
A file cannot disagree with anything, because it cannot choose. These two versions differ only in
the second sentence.
...
A writer who forces a sentence into the active voice without naming an actor that can choose makes
the nearest noun its subject. This project moved the subject to the wrong actor in these rewrites.
...
Each one changed what the sentence demanded. These moves happen during a style pass, because the
writer then counts words and does not read for meaning.
